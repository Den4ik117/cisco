...
@section('title', 'Марафон CISCO')

@section('content')
    <div class="flex flex-col gap-4">
        <page-header href="{{ route('index') }}" text="Марафон по CISCO"></page-header>
        ...
        <form class="mt-10 px-4" action="{{ route('marathons.store') }}" method="POST">
            @csrf
        ...
        <form class="px-4" action="{{ route('marathons.store') }}" method="POST">
            @csrf
